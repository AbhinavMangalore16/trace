...
const MagneticButton = React.forwardRef<HTMLElement, MagneticButtonProps>(
  ({ className, children, as: Component = "button", ...props }, forwardedRef) => {
    ...
    useEffect(() => {
      ...
      gsap.set(element, { willChange: "transform", force3D: true });
      ...
      const rxTo = gsap.quickTo(element, "rotationX", { duration: 0.3, ease: "power2.out" });
      const ryTo = gsap.quickTo(element, "rotationY", { duration: 0.3, ease: "power2.out" });
...
export function CinematicFooter() {
  ...
  useEffect(() => {
    ...
    if (prefersReducedMotion) {
      gsap.set([giantTextEl, headingEl, linksEl], {
        clearProps: "all",
        opacity: 1,
        y: 0,
        scale: 1,
        ...
        <footer className="cinematic-footer-wrapper fixed bottom-0 left-0 flex h-screen w-full flex-col justify-between overflow-hidden bg-black text-foreground">
          ...
          <div className="relative z-20 flex w-full flex-col items-center justify-between gap-6 px-6 pb-8 md:flex-row md:px-12">
            ...
            {/* <div className="footer-glass-pill order-1 flex cursor-default items-center gap-2 rounded-full border-border/50 px-6 py-3 md:order-2">
              ...
              <span className="animate-footer-heartbeat text-sm text-destructive md:text-base">heart</span>
              <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground md:text-xs">by</span>
              <span className="ml-1 text-xs font-black tracking-normal text-foreground md:text-sm">Abhinav</span>
            </div> */}
